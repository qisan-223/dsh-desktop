...
function blockText(block) {
	...
	if (block.type === "image") return block.description === void 0 ? "" : `[图片内容]\n${block.description}`;
...
function flattenText(message) {
	return message.content.filter((block) => block.type !== "tool-result").map(blockText).join("");
}
...
function contentHasUndescribedImage(content) {
	return content.some((block) => block.type === "image" && block.description === void 0 || block.type === "tool-result" && contentHasUndescribedImage(block.content));
}
function foldBlock(block) {
	if (block.type === "image" && block.description !== void 0) return {
		type: "text",
		text: `[图片内容]\n${block.description}`
	};
	if (block.type === "tool-result") return {
		...block,
		content: block.content.map(foldBlock)
	};
	return block;
}
/** Replace bridge-described images with text before a text-only provider sees them. */
function foldDescribedImages(messages) {
	return messages.map((message) => ({
		...message,
		content: message.content.map(foldBlock)
	}));
}
	...
	async *stream(options) {
		...
		try {
			...
			try {
				...
				const textOnly = !model.input.includes("image");
				if (textOnly && containsImage && options.messages.some((message) => contentHasUndescribedImage(message.content))) throw new LlmError(`pi-ai model "${model.id}" does not support image input`, "UNSUPPORTED_CONTENT");
				const effectiveOptions = textOnly && containsImage ? {
					...options,
					messages: foldDescribedImages(options.messages)
				} : options;
				const effectiveContainsImage = effectiveOptions.messages.some((message) => contentHasImage(message.content));
				const attachments = effectiveContainsImage ? this.config.resolveAttachments?.() : void 0;
				if (effectiveContainsImage && attachments === void 0) throw new LlmError("pi-ai image input requires the durable attachment service", "UNSUPPORTED_CONTENT");
				const context = attachments === void 0 ? toPiContext(effectiveOptions) : await toPiContext(effectiveOptions, attachments);
