...
import { contentHasUndescribedImage, foldDescribedImages, toPiContext } from "./context.js";
    ...
    async *stream(options) {
        ...
        try {
            ...
            try {
                ...
                if (textOnly && containsImage && options.messages.some(message => contentHasUndescribedImage(message.content))) {
                    throw new LlmError(`pi-ai model "${model.id}" does not support image input`, 'UNSUPPORTED_CONTENT');
                }
                const effectiveOptions = textOnly && containsImage
                    ? { ...options, messages: foldDescribedImages(options.messages) }
                    : options;
                const effectiveContainsImage = effectiveOptions.messages.some(message => contentHasImage(message.content));
                const attachments = effectiveContainsImage ? this.config.resolveAttachments?.() : undefined;
                if (effectiveContainsImage && attachments === undefined) {
                    throw new LlmError('pi-ai image input requires the durable attachment service', 'UNSUPPORTED_CONTENT');
                }
                const context = attachments === undefined
                    ? toPiContext(effectiveOptions)
                    : await toPiContext(effectiveOptions, attachments);
