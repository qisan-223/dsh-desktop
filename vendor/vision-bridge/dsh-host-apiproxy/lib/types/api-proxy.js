...
import { describeImage, resolveVisionApiKey } from "./vision-bridge.js";
...
import { InvalidPresetIdError, PresetExistsError, PresetMountError, PresetNotWritableError, resolveSessionPreset, SETTINGS_NAMESPACE as AGENT_PRESET_SETTINGS_NAMESPACE, UnknownPresetError, } from '@deepseek-ai/dsh-agent-presets';
...
function imageBridgeEnabled(ctx) {
    ...
    if (settings === undefined)
        return true;
    const section = settings.get(settingsNamespace(IMAGE_BRIDGE_SETTINGS_NAMESPACE));
    return section?.[IMAGE_BRIDGE_ENABLED_FIELD] !== false;
}
...
async function bridgePromptImages(ctx, content) {
    ...
    if (apiKey === undefined) {
        throw new AttachmentError('[VB-2026-08-14] The current model cannot take images, and no vision-bridge API key is configured'
            + ' (store GLM_VISION_API_KEY in the credentials document or export it).', 'VISION_BRIDGE_NO_KEY');
    }
    const parts = [];
    for (const part of content) {
        ...
        // Keep the image itself (persisted as a durable attachment so the
        // conversation shows its thumbnail) and carry the text description in a
        // hidden field the serializer folds into model input but the UI never
        // renders.
...
async function durablePromptContent(ctx, content) {
    ...
    for (const item of prepared) {
        ...
        blocks.push({
            type: 'image',
            attachment,
            ...item.part.description === undefined ? {} : { description: item.part.description },
        });
    }
    return blocks;
}
...
async function bridgeSessionImages(ctx, agent) {
    ...
    for (const seq of surfaceNodes) {
        ...
        if (event?.type !== 'user/message' || !contentHasUndescribedImage(event.data.content))
            continue;
        const content = [];
        let changed = false;
        for (const block of event.data.content) {
            ...
            if (described?.type !== 'image' || described.description === undefined) {
                throw new AttachmentError('The vision bridge did not return an image description.', 'VISION_BRIDGE_FAILED');
            }
            content.push({ ...block, description: described.description });
            changed = true;
        }
        if (!changed)
            continue;
        agent.session.append('user/message', { ...event.data, content }, {
...
export function createApiProxy(ctx, defaults) {
    ...
    function exposedNamespaces() {
        ...
        for (const ns of WEB_SETTINGS_NAMESPACES)
            exposed.add(ns);
        for (const ns of PRODUCT_SETTINGS_NAMESPACES)
            exposed.add(ns);
        return exposed;
    }
                ...
                return serializeImageAdmission(found.agent, async () => {
                    try {
                        ...
                        const pendingImage = [...found.agent.inbox.nextTurn, ...found.agent.inbox.nextStep]
                            .some(message => contentHasUndescribedImage(message.content));
                        if (pendingImage || messagesHaveUndescribedImage(found.agent.session.deriveMessages())) {
                            ...
                            if (!info.inputModalities?.includes('image')) {
                                ...
                                // Pending images are not durable attachments yet, so they cannot
                                // be bridged during a model switch. Durable history can be
                                // prepared when the vision bridge is configured.
                                if (pendingImage) {
                                    return err(request, {
                                        code: 'model-unavailable',
                                ...
                                if (await resolveVisionApiKey(ctx) === undefined) {
                                    return err(request, {
                                        code: 'model-unavailable',
                                ...
                                // Prepare existing durable images before committing the switch.
                    ...
                    try {
                        ...
                        if (hasImage) {
                            ...
                            if (!modelInfo.inputModalities?.includes('image')) {
                                // The model cannot see images: convert them to text descriptions
                                // through the vision bridge instead of refusing the prompt.
                                if (!imageBridgeEnabled(ctx)) {
                                    throw new AttachmentError('The selected model cannot accept images; enable the image understanding bridge or select an image-capable model.', 'VISION_BRIDGE_DISABLED');
                                }
