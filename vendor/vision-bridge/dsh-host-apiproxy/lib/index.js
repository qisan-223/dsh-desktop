...
//#region lib/types/vision-bridge.js
/**
...
*/
async function resolveVisionApiKey(ctx) {
	const credentials = ctx.get("credentials");
	const fromService = credentials === void 0 ? void 0 : (await credentials.resolve(credentialRef("GLM_VISION_API_KEY")))?.value;
	if (fromService !== void 0 && fromService.length > 0) return fromService;
...
function textOf(body) {
	...
	if (Array.isArray(content)) {
		const joined = content.filter((part) => typeof part === "object" && part !== null).map((part) => part.text ?? "").join("");
	...
	return new Promise((resolvePromise, rejectPromise) => {
		const payload = JSON.stringify({
			...
			messages: [{
				...
				content: [{
					type: "image_url",
					image_url: { url: `data:${mediaType};base64,${Buffer.from(data).toString("base64")}` }
				}, {
					type: "text",
		...
		const req = request(VISION_URL, {
			...
			res.on("end", () => {
				...
				if (res.statusCode !== 200) {
					rejectPromise(/* @__PURE__ */ new Error(`vision API HTTP ${res.statusCode}: ${raw.slice(0, 500)}`));
					return;
				}
				try {
		...
		req.on("timeout", () => req.destroy(/* @__PURE__ */ new Error("vision API request timed out")));
...
*/
async function describeImage(apiKey, data, mediaType) {
	let lastError;
	for (const model of VISION_MODELS) try {
...
/** Durable product setting controlling the optional text-model image bridge. */
const IMAGE_BRIDGE_SETTINGS_NAMESPACE = "ui-conversation";
const IMAGE_BRIDGE_ENABLED_FIELD = "imageBridgeEnabled";
/** Read the image bridge preference without coupling Host code to the client package. */
function imageBridgeEnabled(ctx) {
	const settings = ctx.get("settings");
	if (settings === void 0) return true;
	return settings.get(settingsNamespace(IMAGE_BRIDGE_SETTINGS_NAMESPACE))?.[IMAGE_BRIDGE_ENABLED_FIELD] !== false;
}
...
/**
* Vision bridge: make pasted images usable by a text-only model.
*
* Invoked from prompt admission when the selected model's `inputModalities`
* exclude `image`. Each image part is sent to the Zhipu GLM vision API and the
* returned description is attached to the image part as a hidden
* `description` field — the image itself is KEPT (durablePromptContent later
* persists it, so the conversation shows a thumbnail) and NO visible text
* block is emitted. The text-only serializer (llm-deepseek serialize.ts)
* folds `description` into model input, and selectModel only rejects images
* lacking a description. See ImageBlock.description in dsh-llm for the full
* contract.
*/
async function bridgePromptImages(ctx, content) {
	const apiKey = await resolveVisionApiKey(ctx);
	if (apiKey === void 0) throw new AttachmentError("[VB-2026-08-14] The current model cannot take images, and no vision-bridge API key is configured (store GLM_VISION_API_KEY in the credentials document or export it).", "VISION_BRIDGE_NO_KEY");
	const parts = [];
	for (const part of content) {
		...
		let description;
		try {
			description = await describeImage(apiKey, decodeBase64(part.data), part.mediaType);
		} catch (error) {
			throw new AttachmentError(`Unable to describe the pasted image through the vision bridge: ${error instanceof Error ? error.message : String(error)}`, "VISION_BRIDGE_FAILED");
		}
		parts.push({
			...part,
			description
...
async function durablePromptContent(ctx, content) {
	...
	for (const item of prepared) {
		...
		blocks.push({
			type: "image",
			attachment,
			...item.part.description === void 0 ? {} : { description: item.part.description }
		});
	}
	return blocks;
}
...
/** True when content carries an image without a text description (a raw vision image a text-only model cannot reason over). */
function contentHasUndescribedImage(content) {
	return content.some((block) => block.type === "image" && block.description === void 0);
}
/** True when the current model-visible surface contains an undescribed image. */
function messagesHaveUndescribedImage(messages) {
	return messages.some((message) => contentHasUndescribedImage(message.content));
}
/**
* Describe raw images already persisted in a session before switching to a
* text-only model. The replacement keeps the same message id and attachment,
* so the UI thumbnail remains intact while the text-only serializer gains the
* hidden description field it needs.
*/
async function bridgeSessionImages(ctx, agent) {
	const events = agent.session.events;
	const surfaceNodes = [...agent.session.surface.nodes];
	for (const seq of surfaceNodes) {
		const event = events[seq];
		if (event?.type !== "user/message" || !contentHasUndescribedImage(event.data.content)) continue;
		const content = [];
		let changed = false;
		for (const block of event.data.content) {
			if (block.type !== "image" || block.description !== void 0) {
				content.push(block);
				continue;
			}
			const stored = await ctx.attachments.readImage(block.attachment);
			const [described] = await bridgePromptImages(ctx, [{
				type: "image",
				mediaType: stored.ref.mediaType,
				data: Buffer.from(stored.data).toString("base64")
			}]);
			if (described?.type !== "image" || described.description === void 0) throw new AttachmentError("The vision bridge did not return an image description.", "VISION_BRIDGE_FAILED");
			content.push({
				...block,
			...
			async selectModel(request) {
				...
				return serializeImageAdmission(found.agent, async () => {
					try {
						...
						const pendingImage = [...found.agent.inbox.nextTurn, ...found.agent.inbox.nextStep].some((message) => contentHasUndescribedImage(message.content));
						if (pendingImage || messagesHaveUndescribedImage(found.agent.session.deriveMessages())) {
							if (!(await ctx.llm.resolveModelInfo(resolved.provider, resolved.model)).inputModalities?.includes("image")) {
								if (!imageBridgeEnabled(ctx)) return err(request, {
									code: "model-unavailable",
									message: `Model "${resolved.model}" does not accept image input; enable the image understanding bridge or select an image-capable model.`,
									details: {
										provider,
										model
									}
								});
								if (pendingImage) return err(request, {
									code: "model-unavailable",
									message: `Model "${resolved.model}" does not accept image input, but this session has a pending image; select an image-capable model or send it first.`,
									details: {
										provider,
										model
									}
								});
								if (await resolveVisionApiKey(ctx) === void 0) return err(request, {
									code: "model-unavailable",
									message: `Model "${resolved.model}" does not accept image input, but this session already contains images; configure GLM_VISION_API_KEY or select an image-capable model.`,
									details: {
										provider,
										model
									}
								});
								await bridgeSessionImages(ctx, found.agent);
							}
						}
						const selected = {
				...
				const admit = async () => {
					try {
						...
						if (hasImage) {
							...
							if (!(await ctx.llm.resolveModelInfo(current.provider, current.model)).inputModalities?.includes("image")) {
								if (!imageBridgeEnabled(ctx)) throw new AttachmentError("The selected model cannot accept images; enable the image understanding bridge or select an image-capable model.", "VISION_BRIDGE_DISABLED");
								effectiveContent = await bridgePromptImages(ctx, content);
							}
						}
						const message = createUserMessage({
							content: await durablePromptContent(ctx, effectiveContent),
							source
						});
