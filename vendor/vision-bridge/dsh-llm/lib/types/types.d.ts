...
export interface ImageBlock {
    ...
    /**
     * Optional vision-model text description ("hidden description" bridge).
     *
     * When the active model is text-only (e.g. DeepSeek), the api-proxy prompt
     * admission describes each pasted image through the GLM vision API and
     * stores the text HERE instead of emitting a visible text block. The UI
     * renders only `attachment` (thumbnail), so the description never shows;
     * text-only serializers (llm-deepseek serialize.ts) fold this field into
     * the model input, and the selectModel admission check treats images
     * WITHOUT a description as the only ones a text-only model cannot serve.
     */
    description?: string;
}
