 ...
 *
 * The DeepSeek chat-completions route is text-only, so image blocks never go
 * on the wire. Images pasted through the vision bridge carry a hidden
 * `description` (see ImageBlock.description / bridgePromptImages in
 * api-proxy); this function folds that text into the model input so the
 * model can reason about the image, while the UI keeps showing only the
 * thumbnail. Images WITHOUT a description contribute nothing (the text-only
 * route cannot serve them — such a message should never reach here, because
 * selectModel rejects undescribed images for text-only models).
 */
...
/** Image blocks carry an optional description folded into wire text by flattenText, so the text-only route accepts them. */
function assertTextOnly(_blocks) {
    // No rejection: descriptions are folded into text by flattenText above.
}
